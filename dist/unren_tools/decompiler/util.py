...
def _param_name_default(p):
    """Normaliza un parameter a (name, default).
    Ren'Py < 8.0: p es tuple (name, default)
    Ren'Py 8.0+: p es objeto Parameter con atributos name/default."""
    try:
        # Tuple-style (Ren'Py antiguo)
        return p[0], p[1]
    except Exception:
        pass
    # Object-style (Ren'Py 8.0+)
    name = getattr(p, 'name', None)
    # 'default' puede tener varios nombres: default, value, arg
    default = getattr(p, 'default', None)
    if default is None:
        default = getattr(p, 'value', None)
    # En Parameter de Ren'Py 8 moderno, si no hay default, puede ser un sentinel
    # Detectar "no hay default" vs "default=None"
    try:
        _kind = getattr(p, 'kind', None)
        _has_default = getattr(p, 'has_default', None)
        if _has_default is False:
            default = None
    except Exception:
        pass
    return name, default
...
def reconstruct_paraminfo(paraminfo):
    ...
    sep = First("", ", ")
    _params = getattr(paraminfo, 'parameters', None) or []
    _positional_names = getattr(paraminfo, 'positional', None) or []
    _extrapos = getattr(paraminfo, 'extrapos', None)
    _extrakw = getattr(paraminfo, 'extrakw', None)
    # Normalizar cada parametro a (name, default)
    _normalized = []
    for p in _params:
        try:
            _normalized.append(_param_name_default(p))
        except Exception:
            pass
    positional = [i for i in _normalized if i[0] in _positional_names]
    nameonly = [i for i in _normalized if i not in positional]
    for parameter in positional:
        rv.append(sep())
        rv.append(str(parameter[0]))
        if parameter[1] is not None:
            rv.append("=%s" % parameter[1])
    if _extrapos:
        rv.append(sep())
        rv.append("*%s" % _extrapos)
    if nameonly:
        if not _extrapos:
            rv.append(sep())
            rv.append("*")
